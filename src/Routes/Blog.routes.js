                                                        ...
                                                        .then(res_author_name => {
                                                            if (res_author_name !== null || res_author_name !== undefined) {
                                                                if (res_author_name?.length > 0) {
                                                                    res_author_name?.map(item => authors_username.push(item));
                                                                }
                                                            }
                                                        });
                                                } catch (error) {
                                                    authors_username.push();
                                                }
                                                const blogs_arr = [];
                                                blog_res.map(item => {
                                                    const p_author = authors_username?.filter(a_info => a_info?._id?.toString() === item?.author?.toString())?.[0]?.username;
                                                    const blog_item = {};
...
router.get('/', verifyJWTHeaderIA, async (req, res) => {
    ...
    const invalid_arr_tags = invalid_arr_checker(tags);
    const processed_tags = JSON.parse(invalid_arr_tags);
    const pagination_index = req.query.pagination_index;
    const query_f_i = pagination_indexer(pagination_index, 50)?.first_index;
    const query_l_i = pagination_indexer(pagination_index, 50)?.last_index;
